...
{
    public class CompletedTransactionViewComponent : ViewComponent
    {
        ...
        public CompletedTransactionViewComponent(TransactionHeaderRepo _transactionHeaderRepo, UserManagementRepo _userRepo, IHttpContextAccessor _context)
        {
        ...
        {
            var TransactionList = transactionHeaderRepo.GetCompletedTransactionsHistory(CustomerId);

            var model = new TransactionHistoryViewModel();
            model.ListTransaction = TransactionList;

            return View("CompletedTransaction", model);
        }
    }
}
